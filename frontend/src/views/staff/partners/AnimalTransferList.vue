...
const loadTransfers = async () => {
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to load transfers', life: 3000 })
  } finally {
    loading.value = false
  }
}
  ...
  confirm.require({
    ...
    accept: async () => {
      try {
        ...
        toast.add({ severity: 'success', summary: 'Success', detail: t('partner.transferDeleted'), life: 3000 })
        loadTransfers()
      } catch (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to delete transfer', life: 3000 })
      }
    }
  })
}
